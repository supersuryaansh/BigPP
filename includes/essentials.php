...
function ph_userEmailVerify($userEmailToVerify){
    ...
    // prepare and bind
    $stmt = $conn->prepare("SELECT userEmail from profileHunt_Users WHERE userEmail = ?");
...
function ph_userData($userEmail){
    ...
    // prepare and bind
    $stmt = $conn->prepare("SELECT * from profileHunt_Users WHERE userEmail = ?");
...
###
  function ph_userCreate($userEmail,$userPassword){
    ...
    //create user account
    $stmt = $conn->prepare(
        "INSERT INTO profileHunt_Users(userName,userEmail,userPassword,joinDate) VALUES(NULL,?,?,sysdate())"
    );
